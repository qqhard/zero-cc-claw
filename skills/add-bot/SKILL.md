...
- TaskCreate("Name and configure new agent")
...
3. **Name and configure**:
   - **Agent name**: Suggest 3-5 mythology names (different from existing bots). Explain what this agent will specialize in, or let it be a general assistant.
   - **Role** (optional): Ask if this agent has a specific focus — e.g. "research assistant", "writing editor", "code reviewer". This goes into its CLAUDE.md Role section.

4. **Generate files** in `<parent>/<agent-name-lowercase>/`:
   - Copy `$CLAUDE_PLUGIN_ROOT/template/CLAUDE.md` → `CLAUDE.md`, fill in agent name, user info (reuse `USER.md` from sibling bot), role.
...
5. **Register with supervisor**: Add the new bot to the parent's `ecosystem.config.cjs` — add a new entry in the apps array with the new bot's tmux session name. Restart supervisor: `pm2 restart supervisor`.
